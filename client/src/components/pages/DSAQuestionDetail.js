...
const DSAQuestionDetail = () => {
  const { id } = useParams();
  const question = DSA_QUESTIONS.find(q => q.id === id);
  if (!question) return <Container><Typography>Question not found.</Typography></Container>;
  return (
    <Container maxWidth="md" sx={{ mt: 4, mb: 4 }}>
      <Typography variant="h4" gutterBottom>{question.title}</Typography>
      <Typography variant="body1" color="text.secondary" paragraph>{question.description}</Typography>
      <Paper sx={{ p: 2, mb: 2 }}>
        <Typography variant="subtitle1" gutterBottom>Explanation</Typography>
        <Typography variant="body2" paragraph>{question.explanation}</Typography>
        <Typography variant="subtitle1">Sample Input</Typography>
        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>{question.input}</Typography>
        <Typography variant="subtitle1">Expected Output</Typography>
        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>{question.output}</Typography>
        <Typography variant="subtitle1" sx={{ mt: 2 }}>C Code</Typography>
        <Box sx={{ bgcolor: '#f5f5f5', p: 1, borderRadius: 1, fontFamily: 'monospace', fontSize: 13, whiteSpace: 'pre-wrap' }}>
          <pre style={{ margin: 0 }}>{question.code}</pre>
        </Box>
      </Paper>
      <Box sx={{ mt: 4 }}>
        <Typography variant="h6" gutterBottom>Try it Yourself</Typography>
        <Interpreter initialCode={question.code} />
      </Box>
    </Container>
  );
};

export default DSAQuestionDetail; 